...
def test_linearity_superposition(backend):
    ...
    Wx = cwt(x, wavelet="morlet", scales=scales, fs=FS, backend=backend).W
    Wy = cwt(y, wavelet="morlet", scales=scales, fs=FS, backend=backend).W
    Wxy = cwt(a * x + b * y, wavelet="morlet", scales=scales, fs=FS, backend=backend).W

    np.testing.assert_allclose(Wxy, a * Wx + b * Wy, atol=1e-10)


# --- Frequency localization ---

def test_energy_peak_near_correct_scale(backend):
    ...
    # Peak should be within 15% of the true frequency
    assert abs(peak_freq - f0) / f0 < 0.15, (
        f"Expected peak near {f0} Hz, got {peak_freq:.2f} Hz"
    )


def test_energy_peak_shifts_with_frequency(backend):
    ...
    r1 = cwt(x1, wavelet="morlet", scales=scales, fs=FS, backend=backend)
    r2 = cwt(x2, wavelet="morlet", scales=scales, fs=FS, backend=backend)

    energy1 = np.sum(np.abs(r1.W) ** 2, axis=1)
    energy2 = np.sum(np.abs(r2.W) ** 2, axis=1)
...
def test_two_component_peaks(backend):
    ...
    # The two highest-energy peaks should bracket f1 and f2
    assert peak_freqs[0] < f2 and peak_freqs[-1] > f1


# --- nv parameter ---

def test_nv_controls_scale_count(backend):
    """Higher nv should produce more scales."""
    x = _pure_tone(32.0)
    r16 = cwt(x, wavelet="morlet", scales="auto", fs=FS, nv=16, backend=backend)
    r32 = cwt(x, wavelet="morlet", scales="auto", fs=FS, nv=32, backend=backend)
    assert r32.W.shape[0] > r16.W.shape[0]


def test_nv_32_has_expected_density(backend):
    """With nv=32, scales should step by factor 2^(1/32) ≈ 1.022 between neighbors."""
    x = _pure_tone(32.0)
    result = cwt(x, wavelet="morlet", scales="auto", fs=FS, nv=32, backend=backend)
